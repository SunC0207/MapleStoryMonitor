...
class GameMonitorMainWindow:
    ...
    def __init__(self):
        ...
        self.auto_potion_thread = None
        self.auto_potion_running = False
        ...
        # 監控標籤頁
        self.tabs = {}
        self.tabs_names = ["HP", "MP", "EXP", "楓幣", "藥水1", "藥水2", "藥水3", "藥水4", "藥水5", "藥水6", "藥水7",
                           "藥水8"]
    ...
    def _create_overview_tab(self):
        ...
        self.results_frame.pack(side="top", fill="x", expand=False, padx=20, pady=(0, 5))
        ...
        self.ocr_status_frame.pack(side="top", fill="x", padx=20, pady=(0, 5))

        tk.Label(self.ocr_status_frame, text="OCR狀態:", font=('Arial', 10)).pack(side="left", padx=(5, 0))
        self.ocr_status_label = tk.Label(
            self.ocr_status_frame,
            text="初始化中...",
            font=('Arial', 10),
        )
        self.ocr_status_label.pack(side="left", padx=(2, 0))

        # EXP計算器
        self.multi_tracker_frame = ttk.LabelFrame(self.overview_frame, text="多功能追蹤計算器")
        self.multi_tracker_frame.pack(side="top", fill="x", padx=20)

        self.multi_tracker = MultiTrackerWidget(self.multi_tracker_frame,
                                                self.exp_manager,
                                                self.coin_manager,
                                                self.potion_manager)
        self.multi_tracker.pack(fill="x")

        # 設定分隔線
        tk.Frame(self.overview_frame).pack(side="top", fill="both", expand=True)  # 分隔線
    ...
    def _create_status_labels(self):
        ...
        for stat_name in self.tabs_names:
            if self.tab_enabled_vars[stat_name].get():
                ...
                stat_frame.pack(fill="x", pady=0)

                ttk.Label(stat_frame, text=f"{stat_name}:", font=('Arial', 10, 'bold')).pack(side="left", padx=(5, 5))
    ...
    def _update_status_visibility(self):
        ...
        if hasattr(self, 'results_frame'):
            if self.show_status_var.get():
                self.results_frame.pack(side="top", fill="x", expand=False, padx=20, before=self.multi_tracker_frame)
            else:
    ...
    def _start_monitoring(self):
        ...
        self._start_auto_potion()

    def _stop_monitoring(self):
        """停止監控"""
        self._stop_auto_potion()
    ...
    def _load_config(self):
        ...
        if success:
            ...
            self.root.geometry(
                f"{window_size['width']}x{window_size['height']}+{window_position['x']}+{window_position['y']}")
    ...
    def _start_auto_potion(self):
        """啟動自動喝水"""
        if not self.auto_potion_running:
            self.auto_potion_running = True
            self.auto_potion_thread = threading.Thread(target=self._auto_potion_loop, daemon=True)
            self.auto_potion_thread.start()

    def _stop_auto_potion(self):
        """關閉自動喝水"""
        self.auto_potion_running = False
        if self.auto_potion_thread:
            self.auto_potion_thread = None

    def _auto_potion_loop(self):
        """自動喝水迴圈"""
        import keyboard
        import time

        while self.auto_potion_running:
            try:
                if not self.settings_tab.enable_auto_potion.get():
                    time.sleep(0.1)
                    continue

                # 取得當前HP、MP
                hp_status = self.hpmp_manager.hp_percentage
                mp_status = self.hpmp_manager.mp_percentage
                print(f"當前HP: {hp_status}, 當前MP: {mp_status}")

                current_time = time.time()

                # 檢查HP
                if hp_status is not None and hp_status < float(self.settings_tab.health_percent.get()):
                    if current_time - self.settings_tab.last_potion_time[
                        'hp'] >= self.settings_tab.auto_potion_cooldown:
                        key = self.settings_tab.health_potion_key.get()
                        keyboard.press_and_release(key)
                        self.settings_tab.last_potion_time['hp'] = current_time
                        logger.debug(
                            f"自動喝水觸發 - HP: {hp_status}% < {self.settings_tab.health_percent.get()}%")

                # 檢查MP
                if mp_status is not None and mp_status < float(self.settings_tab.mana_percent.get()):
                    if current_time - self.settings_tab.last_potion_time[
                        'mp'] >= self.settings_tab.auto_potion_cooldown:
                        key = self.settings_tab.mana_potion_key.get()
                        keyboard.press_and_release(key)
                        self.settings_tab.last_potion_time['mp'] = current_time
                        logger.debug(
                            f"自動喝水觸發 - MP: {mp_status}% < {self.settings_tab.mana_percent.get()}%")

                time.sleep(0.1)  # 降低CPU使用率

            except Exception as e:
                logger.error(f"自動喝水錯誤: {e}")
                time.sleep(1.0)
